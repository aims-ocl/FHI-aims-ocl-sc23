...
!  NAME
!   evaluate_en_mp2_correlation_energy_v02
!  SYNOPSIS 

      subroutine evaluate_iepa_mp2_correlation_energy_v02 &
...
!     Error variable
      ...
      character(*), parameter :: func &
               = 'evaluate_iepa_mp2_correlation_energy'
!     Igor
...
!      call cpu_time(time_3KS_intg)
      ...
      ! ============================================================
      ! At first, calculate the first-order correction term 'Ec_1st'
      ! ============================================================
      allocate(xc_matr(n_basis,n_basis), stat=info)
      call check_allocation(info, 'xc_matr', func)
      allocate(KS_xc_matr(n_states,n_states,n_spin), stat=info)
      call check_allocation(info, 'KS_xc_matr', func)
      ! prepare KS_xc_matr
      call integrate_xc_matrix(partition_tab, &
          rho, rho_gradient, kinetic_density, l_shell_max, xc_matr)
      call evaluate_KS_xc_matrix(KS_eigenvector, xc_matr, KS_xc_matr)
      deallocate(xc_matr)
      ! prepare HF_ex_matr then the target Ec_1st
      allocate(HF_ex_matr(n_states,n_states), stat=info)
      call check_allocation(info, 'HF_ex_matr', func)
      allocate(Ec_1st(n_states,n_states,n_spin), stat=info)
      call check_allocation(info, 'Ec_1st', func)
      do i_spin = 1, n_spin, 1
        ! evaluate exact exchange matrix in the KS representation
        HF_ex_matr(:,:) = 0.d0
        do i_state3 = 1, n_homo(i_spin), 1
          do i_state2 = 1, n_states, 1
            do i_state = 1, n_states, 1
              HF_ex_matr(i_state, i_state2) = HF_ex_matr(i_state,i_state2) &
                   - ddot( n_loc_prodbas, &
                       ovlp_3KS(1, i_state, i_state3, i_spin), 1, &
                       ovlp_3KS(1, i_state2, i_state3, i_spin), 1 &
                      ) * &
                  occ_numbers(i_state3,i_spin,1)*dble(n_spin)/2.d0
            enddo
          enddo
        enddo
        ...
        do i_state = 1, n_states, 1
          do i_state2 = 1, n_homo(i_spin), 1
            Ec_1st(i_state2,i_state,i_spin) = HF_ex_matr(i_state2, i_state) * &
                (1.0-hybrid_coeff)-KS_xc_matr(i_state2,i_state,i_spin)
          enddo
        enddo
      enddo
      deallocate(KS_xc_matr,HF_ex_matr)
      ! ============================================================
      !write(use_unit,*) "igor debug", Ec_1st
     ...
     if (n_spin.eq.1) then
          ...
          do i_state=i_start_mp2, n_homo(n_spin),1
            do i_state2=i_state,n_homo(n_spin),1
              ...
              vee_matr_os = &
                     ddot( n_loc_prodbas, &
                       ovlp_3KS(1, i_state, i_state, 1), 1, &
                       ovlp_3KS(1, i_state2, i_state2, 1), 1 )
              vee_matr_ss = vee_matr_os - &
                     ddot( n_loc_prodbas, &
                       ovlp_3KS(1, i_state, i_state2, 1), 1, &
                       ovlp_3KS(1, i_state2, i_state, 1), 1 )
              if (use_mpi) then
                  call MPI_ALLREDUCE(vee_matr_os, E_mp2_mpi, 1, &
                         MPI_DOUBLE_PRECISION, &
                         MPI_SUM, mpi_comm_global, mpierr)
                  vee_matr_os = E_mp2_mpi
                  call MPI_ALLREDUCE(vee_matr_ss, E_mp2_mpi, 1, &
                         MPI_DOUBLE_PRECISION, &
                         MPI_SUM, mpi_comm_global, mpierr)
                  vee_matr_ss = E_mp2_mpi
              endif
              ec_1st_ij_os = Ec_1st(i_state,i_state,1) + Ec_1st(i_state2,i_state2,1) - &
                  vee_matr_os
              ec_1st_ij_ss = Ec_1st(i_state,i_state,1) + Ec_1st(i_state2,i_state2,1) - &
                  vee_matr_ss
              ...
              do i_virt=n_lumo(n_spin), n_states,1
                ...
                if(myid.eq.MOD(i_virt,n_tasks)) then
                    !! ===============================================
                    !! Now calculate the single-exciation contribution
                    !! ===============================================
                    !E_mp2_1= (KS_eigenvalue(i_virt,n_spin, 1)) &
                    !  -  (KS_eigenvalue(i_state,n_spin,1) )
                    !E_mp2_a = Ec_1st(i_virt,i_state)
                    !Ec_1st_term_os = Ec_1st_term_os +  &
                    !    E_mp2_a * E_mp2_a / E_mp2_1

                    !E_mp2_2= (KS_eigenvalue(i_virt,n_spin, 1)) &
                    !  -  (KS_eigenvalue(i_state2,n_spin,1) )
                    !E_mp2_b = Ec_1st(i_virt,i_state2)
                    !Ec_1st_term_os = Ec_1st_term_os +  &
                    !    E_mp2_b * E_mp2_b / E_mp2_2
...
!   close a virtual state
              enddo
              if(use_mpi) then
                  !! sync first-order terms
                  !call MPI_ALLREDUCE(Ec_1st_term_os, E_mp2_mpi, 1, &
                  !       MPI_DOUBLE_PRECISION, &
                  !       MPI_SUM, mpi_comm_global, mpierr)
                  !Ec_1st_term_os = E_mp2_mpi
              ...
              call nsolver_iepa_mp2_v02(aux_eri,denominator, &
...
! MPI task distribution
                    if(myid.eq.MOD(i_virt,n_tasks)) then
                        do i_virt2=n_lumo(i_spin2),n_states,1
                          if (i_state.eq.i_state2.and.i_virt.eq.i_virt2.and. &
                              i_spin.eq.i_spin2) then
                              ! set the value to zero for nsolve_iepa subroutine
                              denominator(i_virt,i_virt2) = 1.0d8
                              cycle
                          endif
                  ...
                  if(use_mpi) then
                      ...
                      call MPI_ALLREDUCE(E_mp2_term_os, E_mp2_mpi_os, 1, &
                             ...
                             MPI_SUM, mpi_comm_global, mpierr)
                      E_mp2_term_os = E_mp2_mpi_os
                      !call sync_matrix( numerator_a(n_lumo_min:n_states, &
                      !                  n_lumo_min:n_states),n_unoccupied, &
                      !                  n_unoccupied &
                      !                 )
                      !call sync_matrix( numerator_b(n_lumo_min:n_states, &
                      !                  n_lumo_min:n_states),n_unoccupied, &
                      !                  n_unoccupied &
                      !                 )
                      call sync_matrix( denominator(n_lumo_min:n_states, &
                  ...
                  E_mp2    = E_mp2 - 0.5d0*E_mp2_term
                  E_mp2_ss = E_mp2_ss - 0.5d0*E_mp2_term_ss
                  E_mp2_os = E_mp2_os - 0.5d0*E_mp2_term_os

                  !ec_1st_ij = Ec_1st(i_state,i_state,i_spin) &
                  !+ Ec_1st(i_state2,i_state2,i_spin2)

                  if (myid .eq. 0) then
                     write(use_unit,'(A,I5,I5,I2,I2)') &
                         'The electron pair: ',i_state, i_state2, i_spin, i_spin2
                  endif
      ...
      if(myid.eq.0) then
          ...
          write(use_unit,'(10X,A)') "| SCPT2 calculation comes to finish ...  "
          write(use_unit,'(A)') " "
          write(use_unit,'(10X,A)')"---------------------------------------------------"
          ...
          if (en_shift_type.eq.3 .or. en_shift_type.eq.4) then
              write(use_unit,'(2X,A,1X,F19.8,A,1X,F19.8,A)') &
                                   "| SCPT2 contribution              :" &
...
!   end of if myid
      ...
      end subroutine evaluate_iepa_mp2_correlation_energy_v02
!****** 
