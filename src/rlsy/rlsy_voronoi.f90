...
!!
use rlsy_constants, only: r8,rl_iou,rl_status,rl_huge,rl_hugeint,rl_exitcode_symmetry,rl_exitcode_mpi
use rlsy_helpers, only: tochar,rl_sqnorm,norm2
use rlsy_geometry, only: rl_plane,rl_bounding_sphere_of_box
...
integer, parameter :: max_n_face=60
integer, parameter :: max_n_node=100
integer, parameter :: safe_n_node_per_face=20
...
type dummypolygon
    ...
    !> centroid
    real(r8), dimension(3) :: centroid=-rl_huge
...
function is_point_inside(cell,point,tol) result(inside)
    !> voronoi cell
    class(rl_voronoi_diagram_cell), intent(in) :: cell
...
!> Generate a Voronoi diagram for a set of points with periodic boundary conditions. 
!> The algorithm is geometrical: given a table of distances (taking periodic boundary conditions into account), I slice a polyhedron with the planes that lie at the midpoint between two points. 
!> I stop when the midpoint is outside the bounding sphere of the polyhedron. 
!> This works great for atoms and other things that are more or less uniformly distributed, but will obviously fail for non-periodic cases.
subroutine generate(voro,points,basis,verbosity,cutoff,mw)
    !> diagram
    class(rl_voronoi_diagram), intent(out) :: voro
    ...
    if ( verbosity .gt. 0 ) then
        ...
        t1=0.0_r8
        write(rl_iou,*)
        write(rl_iou,*) 'Generating Voronoi diagram ( ',tochar(size(points,2)),' points)'
    endif
    ...
    if ( present(cutoff) ) then
        ...
        rc=rl_bounding_sphere_of_box(basis)*2.0_r8
    endif

    ! Use a fix cutoff, or guess it.
    if ( verbosity .gt. 0 ) then
        write(rl_iou,*) '... building distance table with cutoff=',tochar(rc,4)
    endif
    ...
    if ( verbosity .gt. 0 ) then
        t1=mpi_wtime()
        write(rl_iou,*) '... got distance table in ',tochar(t1-t0),'s'
        t0=t1
    endif
    ...
    if ( verbosity .gt. 0 ) then
        write(rl_iou,*) '... got tesselation in ',tochar(mpi_wtime()-timer),'s'
    endif
end subroutine
...
subroutine polyhedron_for_one_point(cell,dt,point,tol)
    ...
    type(rl_voronoi_diagram_cell), intent(out) :: cell
    !> the Distance table
    type(rl_distancetable), intent(in) :: dt
    ...
    call rl_return_unique(dr0,cell%node)
    ...
    do i=1,size(face)
        ...
        do j=1,face(i)%n
            ...
            v0=face(i)%r(:,j)
            do k=1,cell%n_node
                if ( rl_sqnorm(cell%node(:,k)-v0) .lt. tol**2 ) then
                    ii=k
                    exit
                endif
            enddo
            if ( ii .gt. 0 ) then
                ...
                call rl_stop_gracefully(['Could not locate point on face.'],rl_exitcode_symmetry)
            endif
        enddo
        ...
        do k=2,imax
            if ( abs(dt%particle(point)%d(k)*0.5_r8+face(i)%plane%p) .lt. tol ) then
                ...
                v0=v0/norm2(v0)
                if ( rl_sqnorm(v0-face(i)%plane%normal) .lt. tol**2 ) then
        ...
        if ( ii .gt. 0 ) then
            ...
            call rl_stop_gracefully(['Could not locate neighbour for face.'],rl_exitcode_symmetry)
        endif
    enddo
    ...
    f0=0.0_r8
    do i=1,cell%n_node
        f0=max(f0,rl_sqnorm(cell%node(:,i)))
    enddo
...
subroutine slice_polyhedron(n_face,face,plane,rmax,tol,n_changed)
    ...
    il1: do i=1,ctr_new_face
        ...
        !     call rl_stop_gracefully(['New points not on plane'],rl_exitcode_symmetry,__FILE__,__LINE__)
        ! endif
    ...
    if ( ctr_sliced .gt. 0 .and. l .le. 2 ) then
        call rl_stop_gracefully(['Should have points.'],rl_exitcode_symmetry)
    endif
    ...
    if ( l .gt. 0 ) then
        ...
        enddo
        if ( i_face .eq. 0 ) then
            call rl_stop_gracefully(['Could not find empty spot for new face'],rl_exitcode_symmetry)
        endif
        face(i_face)%n=l
...
subroutine communicate_diagram(voro,mw)
    !> voronoi diagram
    type(rl_voronoi_diagram), intent(inout) :: voro
    !> MPI helper
    ...
    if ( pos .ne. bytes ) then
        call rl_stop_gracefully(['I could not pack properly.'],rl_exitcode_mpi,mw%comm)
    endif

    ! Spread it around
    call mpi_allgatherv(sbuf,bytes,MPI_CHARACTER,rbuf,rnksize,rnkoff,MPI_CHARACTER,mw%comm,mw%error)
    if ( mw%error .ne. 0 ) then
        call rl_stop_gracefully(['mpi_allgatherv exit code: '//tochar(mw%error)],rl_exitcode_mpi,mw%comm)
    endif
    ...
    do i=1,voro%n_point
        ...
        if ( i .ne. j ) then
            call rl_stop_gracefully(['I could not communicate.'],rl_exitcode_mpi,mw%comm)
        endif
...
subroutine build_cube(n_face,face,side)
    ...
    do i=1,n_face
        ! store the points
        face(i)%n=4
        !rl_allocate(polyhedron%face(i)%r(3,4))
