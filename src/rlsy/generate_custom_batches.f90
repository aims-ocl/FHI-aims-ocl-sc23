...
subroutine rl_generate_batches_for_density_update(rlsy_h,n_batches,sym_batches,n_full_points,partition_tab) !,&
    ...
    !> partition_tab
    real(r8), dimension(:), allocatable, intent(out) :: partition_tab

    integer :: ib,ip

    ! Number of batches
    ! n_batches=rlsy_h%grid%n_irr_batch
    ! allocate(sym_batches(b_batches))
    ! do ib=1,rlsy_h%grid%n_irr_batch
    !     sym_batches(ib)%size = rlsy_h%grid%n_point
    ! enddo
