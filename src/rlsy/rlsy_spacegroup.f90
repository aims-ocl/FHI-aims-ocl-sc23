module rlsy_spacegroup
!! Generate and put the spacegroup into a neat container.
use rlsy_constants, only: r8,rl_iou,rl_huge,rl_hugeint,rl_exitcode_param,rl_tol,rl_sqtol,rl_radiantol,&
                          rl_twopi,rl_exitcode_symmetry,rl_exitcode_memory
use rlsy_memtracker, only: rl_memtracker
use rlsy_mpi_helper, only: rl_mpi_helper,rl_stop_gracefully,mpi_wtime
use rlsy_helpers, only: rl_chop,rl_clean_fractional_coordinates,rl_invert3x3matrix,tochar,&
                        rl_determ, norm2
use rlsy_sorting, only: rl_return_unique,rl_qsort
use rlsy_geometry, only: rl_improper_rotation_matrix_from_axis_and_angle,&
                         rl_rotation_matrix_from_axis_and_angle
use rlsy_linalg, only: rl_general_real_eigenvalues_eigenvectors,rl_triplegemm,&
                       rl_invert_real_matrix,rl_real_pseudoinverse
implicit none
...
type rl_spacegroup_operation
    ...
    ! real(r8) :: fwd_alpha=-rl_huge,bwd_alpha=-rl_huge
    ! !> beta angle
    ! real(r8) :: fwd_beta=-rl_huge,bwd_beta=-rl_huge
    ! !> gamma angle
    ! real(r8) :: fwd_gamma=-rl_huge,bwd_gamma=-rl_huge
end type

!> conjugacy classes of the symmetry operations
type rl_symset_conjugacyclass
   !> number of members in class
   integer :: n_member=-rl_hugeint
   !> members
   integer, dimension(:), allocatable :: member
end type

!> irreducible representations
type rl_symset_irreducible_representation
   !> dimension of representation
   integer :: dimension=-rl_hugeint
   !> character per class
...
!> Set of symmetry operations defining a space group
type rl_spacegroup
    !> number of operations
    integer :: n_operation=-rl_hugeint
    !> operations
    ...
    integer :: n_class=-rl_hugeint
    !> how many irreducible representations are there
    integer :: n_irrep=-rl_hugeint
    !> conjugacy classes
    type(rl_symset_conjugacyclass), dimension(:), allocatable :: cc
    !> irreducible represenations
    type(rl_symset_irreducible_representation), dimension(:), allocatable :: ir

    !> how many unique atoms are there
    integer :: n_irreducible_atom=-rl_hugeint
...
! Hard tolerance for what a zero is. Should not matter particularly.
real(r8), parameter :: zerotol=1E-12_r8

contains

!!> Apply an operation to a vector
!pure function rl_operate_on_vector(op,v,reciprocal,inverse,fractional) result(w)
!    !> operation
...
subroutine generate(sym,latticevectors,timereversal,fractional_coordinate,species,symmorphic,tol,verbosity,mw,mem,flavor,nosym)
    !> spacegroup
    class(rl_spacegroup), intent(out) :: sym
    !> lattice vectors
    real(r8), dimension(3,3), intent(in) :: latticevectors
    !> should time-reversal symmetry be enforced?
    logical, intent(in) :: timereversal
    !> fractional coordinates of the atoms
    real(r8), dimension(:,:), intent(in) :: fractional_coordinate
    !> species of the atoms?
    integer, dimension(:), intent(in) :: species
    !> generate only the symmorphic operations?
    logical, intent(in) :: symmorphic
    !> tolerance, in reciprocal distance units
    ...
    !> add a flavor to each point?
    integer, dimension(:), intent(in), optional :: flavor
    !> return only the identity operation, in case I already know there is not symmetry
    logical, intent(in), optional :: nosym

    ! Dummy points, species and flavors
    type(rl_speciesbox), dimension(:), allocatable :: sbox
    ! Dummy operations
    real(r8), dimension(:,:,:), allocatable :: op,fop
    real(r8), dimension(:,:), allocatable :: tr,ftr
    ! Local stuff
    real(r8), dimension(3,3) :: inv_latticevectors
    real(r8), dimension(3,3) :: reciprocal_latticevectors
    real(r8), dimension(3,3) :: inv_reciprocal_latticevectors
    real(r8) :: timer,t0,t1
    ...
    !init: block
        ...
        if ( verbosity .gt. 0 ) then
           ...
           t1=0.0_r8
           write(rl_iou,*) ' '
           write(rl_iou,*) 'Generating spacegroup operations'
        endif
        ...
        ! shorthand for number of points
        np=size(fractional_coordinate,2)

        ! check that number of points make sense?
        if ( size(species) .ne. np ) then
           call rl_stop_gracefully(['You need to have a classification for every point'],&
           rl_exitcode_param,mw%comm)
        endif

        ! If the points have a flavor, make note of that, otherwise set it to nothing.
        allocate(flv(np))
        if ( present(flavor) ) then
           call rl_stop_gracefully(['Future Olle will port flavor to AIMS.'],&
           rl_exitcode_param)
           flv=-1
        else
           flv=0
        ...
        ! Get all the versions of the basis vectors, all will come handy
        inv_latticevectors=rl_invert3x3matrix(latticevectors)
        reciprocal_latticevectors=transpose(rl_invert3x3matrix(latticevectors))
        inv_reciprocal_latticevectors=rl_invert3x3matrix(reciprocal_latticevectors)

        ! count the number of different species
        call rl_return_unique(species,di,mem)
        nspecies=size(di,1)
        allocate(sbox(nspecies))
        ...
        do i=1,np
           do j=1,nspecies
               if ( di(j) .eq. species(i) ) sbox(j)%n=sbox(j)%n+1
           enddo
        enddo
        ! make space
        do i=1,nspecies
           allocate(sbox(i)%r(3,sbox(i)%n))
           allocate(sbox(i)%ind(sbox(i)%n))
           sbox(i)%n=0
        enddo
        ! stuff the boxes
        do i=1,np
           do j=1,nspecies
               if ( di(j) .eq. species(i) ) then
                   sbox(j)%n=sbox(j)%n+1
                   sbox(j)%r(:,sbox(j)%n)=fractional_coordinate(:,i)
        ...
        if ( verbosity .gt. 0 ) then
           t1=mpi_wtime()
           write(rl_iou,*) '... rearranged atoms per species in '//tochar(t1-t0)//'s'
           t0=t1
        endif
    !end block init
    ...
    !spaceops: block
        ...
        ! Find lattice vectors of the smallest cell (niggli-reduced)
        call find_primitive_cell(latticevectors,fractional_coordinate,species,flv,primlat,tol)
        ! A list of the possible operations
        call return_all_operations(op,fop,primlat,latticevectors,tol,killsym)
        if ( verbosity .gt. 0 ) then
            t1=mpi_wtime()
            write(rl_iou,*) '... found '//tochar(size(op,3))//&
                       ' possible proper/improper rotations in '&
                       //tochar(t1-t0)//'s'
            t0=t1
        endif
        ! Get the possible translations
        if ( symmorphic .eqv. .true. ) then
            ...
            call return_possible_translations(tr,ftr,op,latticevectors,inv_latticevectors,&
            fractional_coordinate,species,tol,killsym)
        endif
        if ( verbosity .gt. 0 ) then
            t1=mpi_wtime()
            write(rl_iou,*) '... found '//tochar(size(tr,2))//' possible translations in '//tochar(t1-t0)//'s'
            t0=t1
        endif

        ! Get a first rough count
        l=size(op,3)*size(tr,2)
        allocate(dumop1(3,3,l))
        allocate(dumtr1(3,l))
        allocate(fmp(np))
        fmp=0
        dumop1=0.0_r8
        ...
        do j=1,size(tr,2)
            ! test this operation
            call check_operation(fop(:,:,i),ftr(:,j),sbox,fmp,opvalid,tol)
            ! maybe store it
            if ( opvalid ) then
                l=l+1
                dumop1(:,:,l)=fop(:,:,i)
        ...
        deallocate(tr)
        deallocate(ftr)
        !call mem%deallocate(ftr,persistent=.true.,scalable=.false.)
        ...
        jl: do j=1,l
            ...
            dv(1:3)=v0
            dv(4:12)=reshape(m0,[9])
            do ii=1,k
                if ( sum(abs(dv-dr0(:,ii))) .lt. rl_sqtol*10 ) cycle jl
            enddo
            call check_operation(m0,v0,sbox,fmp,opvalid,tol)
        ...
        if ( verbosity .gt. 0 ) then
           t1=mpi_wtime()
           write(rl_iou,*) '... found '//tochar(k)//' valid operations in '//tochar(t1-t0)//'s'
           t0=t1
        endif

        ! Now store this
        sym%n_operation=k
        allocate(sym%op(sym%n_operation))
        do o=1,sym%n_operation
            ! Actual operation, fractional coordinates. Check that it is not stupid.
            m0=reshape(dr0(4:12,o),[3,3])
            ! Actual translation, in fractional coordinates
            v0=dr0(1:3,o)

            ! Not sure if this is actually a good check. Have to think.
            if ( sum(abs(m0-anint(m0))) .gt. rl_tol ) then
                call rl_stop_gracefully(['Invalid operation. Could never happen, unless you are not using the primitive cell. Which you should.'],&
            ...
            if ( opvalid .eqv. .false. ) then
                call rl_stop_gracefully(['Invalid operation. Could never happen.'],&
                rl_exitcode_symmetry,mw%comm)
            endif

            ! Store fractional operation
            sym%op(o)%fm=anint(m0)
            sym%op(o)%ifm=anint(rl_invert3x3matrix(m0))
            ! Then make Cartesian operations, and check that they are not stupid
            m0=matmul(latticevectors,sym%op(o)%fm)
            m0=matmul(m0,inv_latticevectors)
            sym%op(o)%m=rl_chop( m0, zerotol )
            sym%op(o)%im=transpose(sym%op(o)%m)
            if ( sum(abs(sym%op(o)%im-rl_invert3x3matrix(sym%op(o)%m))) .gt. rl_tol ) then
                call rl_stop_gracefully(['Invalid operation. Could never happen.'],&
                rl_exitcode_symmetry,mw%comm)
            endif
            ! Reciprocal fractional operation
            m0=matmul(inv_reciprocal_latticevectors,sym%op(o)%m)
            m0=matmul(m0,reciprocal_latticevectors)
            sym%op(o)%rfm=rl_chop(m0,zerotol)
            sym%op(o)%irfm=rl_chop( rl_invert3x3matrix(sym%op(o)%rfm),zerotol )
            ! translations
            sym%op(o)%ftr=v0
            sym%op(o)%tr=rl_chop(matmul(latticevectors,v0),zerotol)
            ! the K=F(k,S) function, in Maradudin's impenetrable notation.
            ! Or permutation array if you are a simpler person.
            allocate(sym%op(o)%fmap( np ))
            sym%op(o)%fmap=fmp
        enddo
    ...
    !uniqueatoms: block
        ...
        ! Make some space
        allocate(sym%irr_to_all(sym%n_irreducible_atom))
        allocate(sym%all_to_irr(np))
        allocate(sym%op_all_from_irr(np))
        sym%irr_to_all=0
        sym%all_to_irr=0
        ...
        allocate(sym%multiplication_table(sym%n_operation,sym%n_operation))
        sym%multiplication_table=0
        do i=1,sym%n_operation
        do j=1,sym%n_operation
            ...
            ctr=0
            l=0
            do k=1,sym%n_operation
                if ( sum(abs(m0-sym%op(k)%fm)) .lt. rl_tol ) then
                    v1=rl_clean_fractional_coordinates(v0-sym%op(k)%ftr+0.5_r8)-0.5_r8
                    if ( sum(abs(v1)) .lt. rl_tol ) then
                        l=k
                        ctr=ctr+1
                    endif
                endif
            ...
            if ( ctr .eq. 1 ) then
                ...
                call rl_stop_gracefully(['The group is not a group, but I thought it was.'],&
                rl_exitcode_symmetry,mw%comm)
            endif
        enddo
        enddo
    ...
    ! Final report
    if ( verbosity .gt. 0 ) then
        write(rl_iou,*) '... identified and classified operations in '//tochar(mpi_wtime()-timer)//'s'
        write(rl_iou,*) 'Generated the relevant spacegroup operations.'
    endif
end subroutine

!> Check if an operation is valid
subroutine check_operation(op,tr,sbox,fmap,valid,tolerance)
    !> point operation
    real(r8), dimension(3,3), intent(in) :: op
    !> translation
    real(r8), dimension(3), intent(in) :: tr
    !> points, decomposed into boxes per species
    type(rl_speciesbox), dimension(:), intent(in) :: sbox
    !> mappy stuff
    ...
    do sp=1,size(sbox,1)
        ...
        do i=1,sbox(sp)%n
            ...
            dum(:,i)=rl_clean_fractional_coordinates(v0,sqtol)
        enddo
        ...
        do i=1,sbox(sp)%n
            ...
            do j=1,sbox(sp)%n
                ...
                v0=rl_clean_fractional_coordinates(v0+0.5_r8,sqtol)-0.5_r8
                ! compare
                if ( v0(1)*v0(1)+v0(2)*v0(2)+v0(3)*v0(3) .lt. sqtol ) then
                    match=.true.
...
subroutine return_possible_translations(translations,fractranslations,ops,basis,inversebasis,points,species,tolerance,killsym)
    !> basis
    ...
    ! Set the tolerance a little looser
    fractol=tolerance*10

    ! Quick exit if no symmetry
    if ( killsym ) then
        allocate(translations(3,1))
        allocate(fractranslations(3,1))
        translations=0
        fractranslations=0
        return
    endif
    ...
    do j=i,n
        if ( species(i) .eq. species(j) ) then
            do o=1,size(ops,3)
                ...
                v0=rl_clean_fractional_coordinates(matmul(inversebasis,v0))
                do k=1,3
    ...
    dum(:,1:l)=rl_clean_fractional_coordinates(dum(:,1:l))
    dum(:,1:l)=rl_chop(dum(:,1:l),fractol)
    dum(:,1:l)=rl_clean_fractional_coordinates(dum(:,1:l))

    ! Now keep only the unique translations, no point in having millions of them.
    call rl_return_unique(dum(:,1:l),fractranslations,fractol)

    allocate(translations(3,size(fractranslations,2)))
...
subroutine return_all_operations(ops,fracops,primbasis,basis,tolerance,killsym)
    !> basis
    ...
    ! Quick exit if we want no symmetry
    if ( killsym ) then
        allocate(ops(3,3,1))
        allocate(fracops(3,3,1))
        ops=0
        fracops=0
        do i=1,3
            ops(i,i,1)=1
    ...
    caninvbasis=rl_invert3x3matrix(canbasis)
    inversebasis=rl_invert3x3matrix(basis)
    ...
    do i9=1,3
        ...
        if ( norm2( matmul( m,transpose(m) )-ident ) .lt. tolerance ) then
            k=k+1
            bflist(:,:,k)=m
        endif
        ! done when I hit 48
...
subroutine figure_out_what_operations_are(sym)
    !> the operations
    type(rl_spacegroup), intent(inout) :: sym
    !
    integer :: i,j,k,o
    integer :: fold
    ...
    do o=1,sym%n_operation
        ...
        eta=rl_determ(m)        ! the determinant
        tr=m(1,1)+m(2,2)+m(3,3) ! the trace
        ! The angle can only be pi,pi/2,pi/3,pi/4,pi/6 or zero
        f0=(tr-eta)*0.5_r8
        if ( abs(f0+1.0_r8) .lt. rl_tol ) f0=-1.0_r8
        if ( abs(f0-1.0_r8) .lt. rl_tol ) f0=1.0_r8
        f0=acos( f0 )
        if ( abs(f0) .lt. rl_radiantol ) then
            theta=0.0_r8
            costheta=cos(theta)
            sintheta=sin(theta)
            fold=0
        elseif ( abs(f0-rl_twopi/2.0_r8) .lt. rl_radiantol ) then
            theta=rl_twopi/2.0_r8
            costheta=cos(theta)
            sintheta=sin(theta)
            fold=2
        elseif ( abs(f0-rl_twopi/3.0_r8) .lt. rl_radiantol ) then
            theta=rl_twopi/3.0_r8
            costheta=cos(theta)
            sintheta=sin(theta)
            fold=3
        elseif ( abs(f0-rl_twopi/4.0_r8) .lt. rl_radiantol ) then
            theta=rl_twopi/4.0_r8
            costheta=cos(theta)
            sintheta=sin(theta)
            fold=4
        elseif ( abs(f0-rl_twopi/6.0_r8) .lt. rl_radiantol ) then
            theta=rl_twopi/6.0_r8
            costheta=cos(theta)
            sintheta=sin(theta)
            fold=6
        endif
        ...
        if ( norm2(m-identity) .lt. rl_tol ) then
            whatkind='identity'
        elseif ( norm2(m+identity) .lt. rl_tol ) then
            whatkind='inversion'
        elseif ( abs(eta*tr+1.0_r8) .gt. rl_tol .and. abs(eta*tr-3.0_r8) .gt. rl_tol ) then
            ! Easy cases:
            ...
            if ( eta .gt. 0.0_r8 ) then
                ...
                if ( abs(theta) .lt. rl_radiantol ) then
            ...
            do i=1,3
                ...
                do j=1,size(welldefinedvalues,1)
                    if ( abs(v0(i)-welldefinedvalues(j)) .lt. rl_sqtol ) then
            ...
            do k=-1,1,2
                ...
                v1(3)=k*v0(3)
                if ( eta .gt. 0.0_r8 ) then
                    m1=rl_rotation_matrix_from_axis_and_angle(v1,theta)
                else
                    m1=rl_improper_rotation_matrix_from_axis_and_angle(v1,theta)
                endif
                f0=norm2(m1-m)
                if ( f0 .lt. rl_tol ) then
                    ! got it
                    vec=v1
                    exit signloop
                endif
...
subroutine find_primitive_cell(basis,r,species,flavor,primbasis,tolerance)
    ...
    redloop: do
        ...
        if ( abs(abs(rl_determ(basis0))-abs(rl_determ(basis1))) .lt. tolerance ) then
    ...
    subroutine reduce_cell(latticevectors,r,species,flavor,newlatticevectors,newr,newspecies,newflavor,tol)
        ...
        l=na+3
        allocate(dum(3,na+3))
        dum(:,1:na)=rl_clean_fractional_coordinates(r)
        do i=1,na
        ...
        call rl_return_unique(dum,vecs,tol)
        deallocate(dum)
        nvecs=size(vecs,2)

        ! volume of original cell
        vol=abs(rl_determ(latticevectors))
        ...
        vl3: do k=j+1,nvecs
            ...
            if ( abs(mod(vol/f0,1.0_r8)) .gt. tol ) cycle vl3   ! has to be an integer division
    ...
    subroutine test_cell(oldcell,oldr,oldspecies,oldflavor,cell,valid,newr,newspecies,newflavor,tol)
        !> original cell
        ...
        f0=abs(rl_determ(oldcell)/rl_determ(cell))
        mult=int(anint(abs(rl_determ(oldcell)/rl_determ(cell))))

        ! m0 is a matrix that converts to fractional coordinates in the new cell
        icell=rl_invert3x3matrix(cell)
        ...
        do i=1,oldna
            dumr1(1:3,i)=rl_clean_fractional_coordinates(matmul(m0,oldr(:,i)),sqtol)
            dumr1(4:5,i)=[oldspecies(i),oldflavor(i)]
        enddo
        ! Reduce this to the union of the list, the unique
        call rl_return_unique(dumr1,dumr2,tol)
...
subroutine reduce_basis_to_something_decent(basis,canonical_basis)
    ...
    do i=3,7
        ...
        if ( abs(rl_determ(canonical_basis)) .gt. rl_tol ) exit
    enddo
    if ( rl_determ(canonical_basis) .lt. 0.0_r8 ) then
        canonical_basis=-canonical_basis
    endif
end subroutine

!> Calculate the character table for the space group
subroutine get_character_table(sym,mw,verbosity,mem)
    !> symmetry operations
    class(rl_spacegroup), intent(inout) :: sym
    !> MPI helper
    type(rl_mpi_helper), intent(inout) :: mw
    !> talk a lot?
    integer, intent(in) :: verbosity
    ...
    integer, dimension(sym%n_operation,sym%n_operation) :: multiplication_table
    real(r8) :: timer,t0,t1
    !getclasses: block
        real(r8), dimension(3,3) :: m0
        real(r8), dimension(3) :: v0,v1
        ...
        integer, dimension(sym%n_operation,sym%n_operation) :: dc0
        integer :: i,j,k,l,ctr
    !getchar: block
        ...
        real(r8), dimension(:), allocatable :: vv0
        real(r8) :: f0
        !integer, dimension(:), allocatable :: di
        !integer :: i,j,k,l,ii,jj,iop,jop,kop,ctr
        integer :: ii,jj,iop,jop,kop
    ...
    !init: block
        if ( verbosity .gt. 0 ) then
            ...
            timer=t0
            write(rl_iou,*) ''
            write(rl_iou,*) 'CALCULATING CHARACTER TABLE'
        endif
    !end block init
    ...
    !getclasses: block
        ...
        call rl_return_unique(dc0,conjugacies,mem)
        ...
        allocate(hi(sym%n_operation,sym%n_class))
        di=0
        dj=0
        hi=0
        do i=1,sym%n_class
            call rl_return_unique(conjugacies(:,i),dc1,mem)
            di(i)=size(dc1)     ! size of class
            hi(1:di(i),i)=dc1   ! members of class
            call mem%deallocate(dc1,persistent=.true.,scalable=.false.)
        enddo
        call mem%deallocate(conjugacies,persistent=.true.,scalable=.false.)
        ...
        if ( sum(abs(di)) .ne. 0 ) then
            call rl_stop_gracefully(['Inclomplete conjugacy classes.'],&
            rl_exitcode_symmetry,mw%comm)
        endif
        deallocate(di)
        ...
        if ( verbosity .gt. 0 ) then
            t1=mpi_wtime()
            write(rl_iou,*) '... got '//tochar(sym%n_class)//' conjugacy classes ('//tochar(t1-t0)//'s)'
            t0=t1
        endif
    ...
    !getchar: block
        ...
        if ( verbosity .gt. 0 ) then
            t1=mpi_wtime()
            write(rl_iou,*) '... got CNRS ('//tochar(t1-t0)//'s)'
            t0=t1
        endif
        ...
        if ( verbosity .gt. 0 ) then
            t1=mpi_wtime()
            write(rl_iou,*) '... got joint eigenvectors ('//tochar(t1-t0)//'s)'
            t0=t1
        endif
        ...
        allocate(vv0(sym%n_class))
        allocate(di(sym%n_class))
        dm0=0.0_r8
        dm1=0.0_r8
        vv0=0.0_r8
        di=0
        call rl_invert_real_matrix(right_eigenvectors,inv_right_eigenvectors)
        ...
        do i=1,sym%n_class
            call rl_triplegemm(inv_right_eigenvectors,cnrs0(:,:,i),right_eigenvectors,dm0)
            do j=1,sym%n_class
                dm1(j,i)=dm0(j,j)
                dm0(j,j)=0.0_r8
            enddo
            ...
            if ( sum(abs(dm0)) .gt. 1E-12_r8*sym%n_class*sym%n_class ) then
                call rl_stop_gracefully(['Failed making similarity transform'],&
                rl_exitcode_symmetry,mw%comm)
            endif
        enddo
        ...
        do i=1,sym%n_irrep
            ...
            if ( abs(f0) .lt. 1E-10_r8 ) then
                call rl_stop_gracefully(['Dimension of irrep has to be nonzero'],&
                rl_exitcode_symmetry,mw%comm)
            else
                f0=sqrt(real(sym%n_operation,r8)/f0)
            endif
            if ( abs(f0-anint(f0)) .gt. 1E-9_r8 ) then
                call rl_stop_gracefully(['Dimension of irrep has to be an integer'],&
                rl_exitcode_symmetry,mw%comm)
            else
                f0=anint(f0)
            endif
            ! If nothing failed, here is the dimension stored in vv0
            vv0(i)=f0
        enddo
        ...
        do i=1,sym%n_irrep
            do j=1,sym%n_class
                f0=vv0(i)/real(sym%cc(j)%n_member,r8)
                dm1(j,i)=f0*dm0(j,i)
            enddo
        enddo
        ...
        do i=1,sym%n_irrep
            sym%ir(i)%dimension=int(anint(vv0(i)))
            allocate(sym%ir(i)%character_class(sym%n_class))
            allocate(sym%ir(i)%character_element(sym%n_operation))
        ...
        do i=1,sym%n_irrep
            ...
            enddo
            dm1(i,i)=dm1(i,i)-sym%n_operation
        enddo
        if ( sum(abs(dm1)) .gt. 1E-12_r8*sym%n_class*sym%n_class ) then
            call rl_stop_gracefully(['irreps not orthogonal'],&
            rl_exitcode_symmetry,mw%comm)
        endif

        if ( verbosity .gt. 0 ) then
            t1=mpi_wtime()
            write(rl_iou,*) '... got character table ('//tochar(t1-t0)//'s)'
            ! t0=t1
            ! ! Fancy output
            ! str="+-"
            ! do i=1,sym%n_class
            ...
            ! enddo
            ! write(rl_iou,*) ''
            ! write(rl_iou,"(6X,"//tochar(sym%n_class)//"(2X,I2))") di
            ! write(rl_iou,"(5X,A)") trim(str)
            ! do i=1,sym%n_class
            !     write(rl_iou,"(2X,I2,' |',"//tochar(sym%n_class)//"(2X,I2))") i,int(dm0(:,i))
            ! enddo
            ! write(rl_iou,*) ''
            write(rl_iou,*) 'Done calculting character table ('//tochar(mpi_wtime()-timer)//'s)'
        endif
    !end block getchar
...
subroutine fancy_joint_eigenvector_thing(M,left_eigenvectors,right_eigenvectors)
    ...
    !slv: block
        ...
        integer :: iter,gi,n_vecs,ii,jj,mj
        integer :: n_sm_group
    ...
    !init: block
        ...
        if ( size(M,1) .ne. size(M,2) ) then
            call rl_stop_gracefully(['Can only solve n x n x n problems'],&
            rl_exitcode_param)
        endif
        ...
        do j=1,n
            ...
            if ( sum(abs(mA-mB)) .gt. 1E-13_r8*n*n ) then
                call rl_stop_gracefully(['Matrices have to commute, all of them.'],&
                rl_exitcode_symmetry)
            endif
        enddo
        enddo
        deallocate(mA)
    ...
    !preiter: block
        ...
        j=rl_hugeint
        do i=1,n
            call rl_general_real_eigenvalues_eigenvectors(M(:,:,i),eigenvalues,left_eigenvectors,right_eigenvectors)
        ...
        enddo
        call rl_general_real_eigenvalues_eigenvectors(M(:,:,mi),eigenvalues,left_eigenvectors,right_eigenvectors)
        call sort_and_reorder_eigenvectors(eigenvalues,left_eigenvectors,right_eigenvectors,n_group,group_ctr,group_member)
    ...
    !slv: block
        ...
        iterloop: do iter=1,maxiter
            ...
            if ( iter .eq. maxiter ) then
                ...
                call rl_stop_gracefully(['Recursing too many times, not good.'],&
                rl_exitcode_symmetry)
            endif
            ...
            grloop: do gi=1,n_group
                ...
                enddo
                call rl_real_pseudoinverse(V0,V0i)
                mj=0
                j=rl_hugeint
                ! Find a good matrix to project on.
                do i=1,n
                    call rl_triplegemm(V0i,M(:,:,i),V0,dm0)
                    ! empty matrices don't help.
                    if ( sum(abs(dm0)) .lt. rl_sqtol ) cycle
                    call rl_general_real_eigenvalues_eigenvectors(dm0,sm_egv,sm_left,sm_right)
                    call sort_and_reorder_eigenvectors(sm_egv,&
                    sm_left,sm_right,n_sm_group,sm_ctr,sm_member)
                ...
                if ( mj .eq. 0 ) then
                    ...
                    call rl_stop_gracefully(['Could not split eigenspace, should never happen.'],&
                    rl_exitcode_symmetry)
                else
                    ! This should always happen.
                    call rl_triplegemm(V0i,M(:,:,mj),V0,dm0)
                    call rl_general_real_eigenvalues_eigenvectors(dm0,sm_egv,sm_left,sm_right)
                    call sort_and_reorder_eigenvectors(sm_egv,sm_left,sm_right,n_sm_group,&
                    sm_ctr,sm_member)
                endif
                ...
                call sort_and_reorder_eigenvectors(eigenvalues,&
                left_eigenvectors,right_eigenvectors,n_group,group_ctr,group_member)
                ! Some cleanup
...
subroutine sort_and_reorder_eigenvectors(eigenvalues,left_eigenvectors,&
    right_eigenvectors,n_group,group_ctr,group_member)
    !> eigenvalues
    ...
    do i=1,n
    do j=1,n
        if ( abs(eigenvalues(i)-eigenvalues(j)) .lt. rl_sqtol ) mult(i)=mult(i)+1
    enddo
    enddo
    call rl_qsort(mult,di)
    dc=eigenvalues
    ...
    ul: do i=1,n
        do j=1,n_group
            if ( abs(eigenvalues(i)-dc(j)) .lt. rl_sqtol ) then
                di(j)=di(j)+1
                dj( di(j),j )=i
                cycle ul
            endif
...
function size_in_mem(sym) result(mem)
   !> symmetry operations
   class(rl_spacegroup), intent(in) :: sym
   ...
   if ( allocated(sym%ir) ) then
       mem=mem+size(sym%ir)*storage_size(sym%ir)
       do i=1,size(sym%ir)
...
!!     type(rl_spacegroup_operation), intent(in) :: op1,op2
!!     type(rl_spacegroup_operation), intent(out) :: op3
...
!!     op3%ftr = rl_clean_fractional_coordinates(op3%ftr)
!! end subroutine
!!
...
!!
!!     real(r8), dimension(3) :: v0
!!     if ( sum(abs(op1%fm-op2%fm)) .gt. rl_sqtol ) then
!!         match=.false.
!!         return
!!     endif
!!
!!     v0=rl_clean_fractional_coordinates(op1%ftr-op2%ftr+0.5_r8)-0.5_r8
!!     if ( sum(abs(v0)) .gt. rl_tol ) then
!!         match=.false.
!!         return
!!     endif
!!
