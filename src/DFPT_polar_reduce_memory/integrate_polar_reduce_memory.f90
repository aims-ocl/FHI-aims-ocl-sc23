...
!****s* FHI-aims/integrate_polar_reduce_memory
!  NAME
!   integrate_polar_reduce_memory
!  SYNOPSIS

subroutine integrate_polar_reduce_memory &
...
!  ARGUMENTS
  ...
  real*8, dimension(3), intent(inout) :: polarizability
  !------------------shanghui end add------------------------------------- 
...
!-------------end mpi---------------------------
  ...
  if(use_batch_permutation > 0) then
    ...
    ! wyj: delta_rho_std permute to rho from all MPI tasks
    allocate(first_order_rho(n_spin, n_full_points_work))
        ...
        do i_index = 1, batches_work(i_batch)%size, 1
          ...
          if (partition_tab(i_full_points).gt.0.d0) then
           ...
           do i_coord = 1, 3
              ...
              polarizability(i_coord)=polarizability(i_coord) & 
              +partition_tab(i_full_points)  & 
              *first_order_rho(i_spin,i_full_points)*coord_current(i_coord)
  ...
  ! wyj
  if (n_bp > 0) deallocate(first_order_rho)
  ...
  !if(.not. use_local_index) call sync_vector( polarizability, 3 )
  call sync_vector( polarizability, 3 )
  !-------shanghui end parallel------

end subroutine integrate_polar_reduce_memory
!******
!------------------------------------------------------------------------------------------------------------
