!***h* FHI-aims/KS_optical_properties_tetrahedron
!  NAME
!    KS_optical_properties_tetrahedron
!  SYNOPSIS
module KS_optical_properties_tetrahedron
  !  PURPOSE
  ...
  integer, save :: block_size_moment
  integer, save :: sc_desc_moment(dlen_)
  integer, save :: mb_moment, nb_moment
  integer, save :: mxld_moment, mxcol_moment
  integer, save :: n_my_rows_moment, n_my_cols_moment
  integer, save, allocatable :: l_row_moment(:), l_col_moment(:)
  integer, save, allocatable :: my_row_moment(:), my_col_moment(:)
  ...
  ! YY
  real*8,     allocatable :: interband_polarizability_reset_im(:,:)

  ! Timings (should be placed in different module, to be honest)
  real*8, save :: tot_time_real_space_mommat = 0.0d0
  real*8, save :: tot_time_bloch_mommat = 0.0d0
  real*8, save :: tot_time_plasma_interband = 0.0d0
  real*8, save :: tot_time_sync_postprocess = 0.0d0
  real*8, save :: tot_clock_time_real_space_mommat = 0.0d0
  real*8, save :: tot_clock_time_bloch_mommat = 0.0d0
  real*8, save :: tot_clock_time_plasma_interband = 0.0d0
  real*8, save :: tot_clock_time_sync_postprocess = 0.0d0

  ! Drivers
  public :: KS_dielectric_calculation_tetrahedron
  ! Utility functions
  public :: calculate_moment_matrix
  public :: calc_moment_w0
  public :: calc_moment_w0_soc
  public :: calculate_plasma_frequency_sq
  public :: calculate_interband_polarizability_tetrahedron
...
contains
  ...
  subroutine KS_dielectric_calculation_tetrahedron &
    ...
    ! arrays for tetrahedron method
    real*8, allocatable :: eigs_symm(:,:,:,:)
    real*8, allocatable :: F_symm(:,:,:,:)
    real*8, allocatable :: eigs_nosymm(:,:,:,:,:,:)
    real*8, allocatable :: F_nosymm(:,:,:,:,:,:)
    real*8, allocatable :: omegas(:)
    integer             :: i_x, i_y, i_z, i_spin
    !

    complex*16, allocatable :: moment_TZ(:,:)
    real*8   :: omega_step

    character(*), parameter :: func = 'KS_dielectric_calculation_tetrahedron'
    ...
    call aims_allocate(omegas, n_omega_reset, "omegas" )
    do i_omega = 1, n_omega_reset
      omegas(i_omega) = (omega_min+(i_omega-1)*omega_step)/hartree
    end do


    call aims_allocate(interband_polarizability_reset, &
         n_omega_reset,n_plot_dielectric, "interband_polarizability_reset" )
    call aims_allocate(interband_polarizability_reset_im, &
         n_omega_reset,n_plot_dielectric, "interband_polarizability_reset_im" )
    ...
    call aims_allocate(eigs_symm, &
         ((n_states_window*(n_states_window+1))/2),n_k_points,n_spin_in,&
           n_plot_dielectric, "eigs_symm" )
    call aims_allocate(F_symm, &
         ((n_states_window*(n_states_window+1))/2),n_k_points,n_spin_in,&
           n_plot_dielectric, "F_symm" )
    call aims_allocate(eigs_nosymm, &
         ((n_states_window*(n_states_window+1))/2),n_k_points_xyz_nosym(1),&
           n_k_points_xyz_nosym(2),n_k_points_xyz_nosym(3),n_spin_in,&
           n_plot_dielectric, "eigs_nosymm" )
    call aims_allocate(F_nosymm, &
         ((n_states_window*(n_states_window+1))/2),n_k_points_xyz_nosym(1),&
           n_k_points_xyz_nosym(2),n_k_points_xyz_nosym(3),n_spin_in,&
           n_plot_dielectric, "F_nosymm" )

    omega_plasma_sq = 0.0d0
    interband_polarizability  = (0.0d0,0.0d0)
    interband_polarizability_reset = (0.0d0,0.0d0)
    interband_polarizability_reset_im = 0.0d0
    ...
    do i_k = 1,n_k_points_task
      ...
      do i_dielectric = 1, n_plot_dielectric
        ...
        call calculate_interband_polarizability_tetrahedron(n_state_min_in, &
             ...
             eigs_symm(:,new_k_point,:,i_dielectric), &
             F_symm(:,new_k_point,:,i_dielectric))
      end do
    ...
    do i_dielectric = 1, n_plot_dielectric
      ...
      call sync_vector_complex (interband_polarizability_reset(:,i_dielectric),&
           n_omega_reset)
      call sync_vector(eigs_symm(:,:,:,i_dielectric),&
            ((n_states_window*(n_states_window+1))/2)*n_k_points*n_spin_in)
      call sync_vector(F_symm(:,:,:,i_dielectric),&
            ((n_states_window*(n_states_window+1))/2)*n_k_points*n_spin_in)
        ...
        do i_y = 1, n_k_points_xyz_nosym(2)
            do i_z = 1, n_k_points_xyz_nosym(3)
              ...
              do i_spin = 1, n_spin_in
                eigs_nosymm(:,i_x,i_y,i_z,i_spin,i_dielectric) = &
                  eigs_symm(:,i_k_point,i_spin,i_dielectric)
                F_nosymm(:,i_x,i_y,i_z,i_spin,i_dielectric) = &
                  F_symm(:,i_k_point,i_spin,i_dielectric)
              end do
            enddo
        enddo
      enddo
    ...
    do i_dielectric = 1, n_plot_dielectric
      call ltispectral(lattice_vector, n_k_points_xyz_nosym(1), n_k_points_xyz_nosym(2), &
                n_k_points_xyz_nosym(3), (n_states_window*(n_states_window+1))/2, &
                eigs_nosymm, F_nosymm, n_omega_reset, omegas, &
                interband_polarizability_reset_im(:,i_dielectric), mpi_comm_global)
      do i_omega = 1, n_omega_reset
        interband_polarizability_reset(i_omega,i_dielectric) = cmplx(0.0d0, interband_polarizability_reset_im(i_omega,i_dielectric))
      end do
    end do
    ...
    if (allocated(eigs_symm)) call aims_deallocate( eigs_symm, "eigs_symm")
    if (allocated(F_symm)) call aims_deallocate( F_symm, "F_symm")
    if (allocated(eigs_nosymm)) call aims_deallocate( eigs_nosymm, "eigs_nosymm")
    if (allocated(F_nosymm)) call aims_deallocate( F_nosymm, "F_nosymm")
    if (allocated(omegas)) call aims_deallocate( omegas, "omegas")
    ...
    ! Fin.
    ! -------------------------------------------------------------------------!
  end subroutine KS_dielectric_calculation_tetrahedron
  !******
  ...
  subroutine calculate_interband_polarizability_tetrahedron( n_state_min_in, &
    ...
    !
    !    modified from calculate_interband_polarizability to calculate the arrays
    !    for tetrahedron integration. YY
    !  USES
    use constants, only: hartree, pi
    use geometry, only: cell_volume
    use runtime_choices, only: n_omega, n_omega_reset, omega_min, omega_max, &
    ...
    real*8,     intent(inout) :: eigs(((n_states_window*(n_states_window+1))/2), n_spin_in)
    real*8,     intent(inout) :: F(((n_states_window*(n_states_window+1))/2), n_spin_in)
    ! INPUTS
    !   o n_state_min_in - Index for minimum state to include in summation
    !   o n_states_window - Total number of states (at current k-point) to
    !     include in the summation
    ...
    character(*), parameter :: func = 'calculate_interband_polarizability_tetrahedron'
      ...
      do n_state = n_state_min_in, n_state_max_in
        do m_state = n_state, n_state_max_in
          ...
          if (KS_eigenvalue(n_state,i_spin) .le. chemical_potential .and. &
            ...
            eigs(i_index,i_spin) = diff_eigen
            F(i_index,i_spin) = diff_im_part
  ...
  end subroutine calculate_interband_polarizability_tetrahedron
  !******
  ...
  subroutine output_optical_properties (i_direction,component_is_diagonal, &
    ...
    broaden_method_name = "Tetrahedron"
...
end module KS_optical_properties_tetrahedron
!******
