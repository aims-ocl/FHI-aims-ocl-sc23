...
module ELSI_SOLVER
   ...
   use ELSI_NTPOLY, only: elsi_init_ntpoly,elsi_solve_ntpoly, elsi_solve_ntpoly_cpscf
   ...
   use ELSI_REDIST, only: elsi_blacs_to_generic_dm,elsi_blacs_to_ntpoly_hs,&
       elsi_blacs_to_ntpoly_hs_real_cpscf, &
...
!>
!! Compute the first_order_density_matrix. Note the intent(inout), everything may be reused
!! in the next call.
!!
subroutine elsi_dm_real_cpscf(eh,ham,ovlp,dm, ebs, &
                                  first_order_ham, &
                                  first_order_ovlp, &
                                  first_order_dm)
   ...
   real(kind=r8), intent(inout) :: first_order_ham(eh%bh%n_lrow,eh%bh%n_lcol) !< first_order_Hamiltonian
   real(kind=r8), intent(inout) :: first_order_ovlp(eh%bh%n_lrow,eh%bh%n_lcol) !< first_order_Overlap
   real(kind=r8), intent(inout) :: first_order_dm(eh%bh%n_lrow,eh%bh%n_lcol) !< first_order_Density matrix
   ...
   character(len=*), parameter :: caller = "elsi_dm_real_cpscf"
   ...
   eh%ph%ill_check = .false.
      ...
      !-----------begin ham, ovlp, dm---------------
      !write(6,*) 'ham_cpscf1:',ham
      !write(6,*) 'ovlp_cpscf1:',ovlp
      !call elsi_blacs_to_ntpoly_hs(eh%ph,eh%bh,ham,ovlp,eh%ph%nt_ham,&
      !     eh%ph%nt_ovlp)
      !call elsi_solve_ntpoly(eh%ph,eh%bh,eh%ph%nt_ham,eh%ph%nt_ovlp,eh%ph%nt_dm)
      !call elsi_ntpoly_to_blacs_dm(eh%ph,eh%bh,eh%ph%nt_dm,dm)
      ...
      !-----------begin first_order_ham, first_order_ovlp, first_order_dm---------------
      !write(6,*) 'ham_cpscf2:',ham
      !write(6,*) 'ovlp_cpscf2:',ovlp
      !here we do not convert ham and ovlp since they have been coverted. 
      call elsi_blacs_to_ntpoly_hs_real_cpscf(eh%ph,eh%bh,first_order_ham,first_order_ovlp,& 
                                       eh%ph%nt_first_order_ham, eh%ph%nt_first_order_ovlp)
      call elsi_solve_ntpoly_cpscf(eh%ph,eh%bh,    & 
           eh%ph%nt_ham,eh%ph%nt_ovlp,eh%ph%nt_dm, & 
           eh%ph%nt_first_order_ham,eh%ph%nt_first_order_ovlp,eh%ph%nt_first_order_dm)
      call elsi_ntpoly_to_blacs_dm(eh%ph,eh%bh,eh%ph%nt_first_order_dm,first_order_dm)
      !-----------end first_order_ham, first_order_ovlp, first_order_dm---------------
