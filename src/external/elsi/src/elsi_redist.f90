...
module ELSI_REDIST
   ...
   public :: elsi_blacs_to_ntpoly_hs_real_cpscf
...
!>
!! Construct first_order_H  in NTPoly format from matrices
!! stored in 2D block-cyclic dense format.
!!
subroutine elsi_blacs_to_ntpoly_hs_real_cpscf(ph,bh,ham_den,ovlp_den,ham_nt,ovlp_nt)
   ...
   character(len=*), parameter :: caller = "elsi_blacs_to_ntpoly_hs_real_cpscf"

   call elsi_get_time(t0)

   if(ph%first_blacs_to_ntpoly_cpscf) then
      if(.not. ph%unit_ovlp) then
         call ConstructEmptyMatrix(ovlp_nt,ph%n_basis,ph%nt_pgrid,.false.)
         call ConstructTripletList(ovlp_list)
      end if
   ...
   write(msg,"(A)") "Finished matrix redistribution (cpscf)"
   call elsi_say(bh,msg)
   write(msg,"(A,F10.3,A)") "| Time :",t1-t0," s"
   call elsi_say(bh,msg)

   ph%first_blacs_to_ntpoly_cpscf = .false.
