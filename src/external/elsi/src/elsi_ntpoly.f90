...
module ELSI_NTPOLY
   ...
   use NTPOLY, only: PM,TRS2,TRS2_cpscf,TRS4,HPCP,EnergyDensityMatrix,LowdinExtrapolate,&
   ...
   public :: elsi_solve_ntpoly_cpscf
...
subroutine elsi_solve_ntpoly(ph,bh,ham,ovlp,dm)
   ...
   if(ph%nt_first) then
      ...
      !----------this is the version to turn off the load_banlance----------
      !ph%nt_options = SolverParameters_t(ph%nt_tol,ph%nt_filter,ph%nt_max_iter,&
      !   ph%nt_output)
...
!>
!! Interface to NTPoly_cpscf.
!!
subroutine elsi_solve_ntpoly_cpscf(ph,bh,ham,ovlp,dm, & 
           first_order_ham,first_order_ovlp,first_order_dm)
   ...
   type(Matrix_ps), intent(inout) :: ovlp
   type(Matrix_ps), intent(inout) :: dm
   type(Matrix_ps), intent(in) :: first_order_ham
   type(Matrix_ps), intent(in) :: first_order_ovlp ! is zero for electric field response
   ...
   write(msg,"(A)") "Starting NTPoly density matrix solver (cpscf)"
   call elsi_say(bh,msg)
   ...
   call TRS2_cpscf(ham,ovlp,ne,dm, first_order_ham, first_order_ovlp, first_order_dm, & 
                   ph%ebs,ph%mu,ph%nt_options)

   call ScaleMatrix(dm,ph%spin_degen)
   call ScaleMatrix(first_order_dm,ph%spin_degen)

   call elsi_get_time(t1)

   write(msg,"(A)") "Finished density matrix purification (cpscf)"
   call elsi_say(bh,msg)
   write(msg,"(A,F10.3,A)") "| Time :",t1-t0," s"
   call elsi_say(bh,msg)
