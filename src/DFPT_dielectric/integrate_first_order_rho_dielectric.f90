!****s* FHI-aims/integrate_first_order_rho_dielectric
!  NAME
!   integrate_first_order_rho_dielectric
!  SYNOPSIS

subroutine integrate_first_order_rho_dielectric &
     (partition_tab_std, & 
      basis_l_max,  &  
...
!  PURPOSE
!  
!  calculate the sum(u,v){DM(1)*phi_u*phi_v},
!  + sum(u,v) {DM(0)* phi_u^1*phi_v + DM(0)*phi_u*phi_v^1 }
...
!  USES
  ...
  use load_balancing
  use pbc_lists
  use prune_matrix_dielectric_mod
  implicit none
...
!  ARGUMENTS
  ...
  !shanghui------------------------------------------------------------------------
  real*8,  dimension(n_hamiltonian_matrix_size), intent(in) :: first_order_density_matrix_sparse
...
!  o density_matrix_sparse -- density_matrix_sparse
!  o first_order_density_matrix_sparse -- first-order density matrix
!
!  OUTPUT
!  o first_order_rho -- first_order density matrix 's contribution to first-order rho.  
!
!  AUTHOR
...
!  real*8 dir_tab(3,n_centers_integrals, n_max_batch_size)
 ...
 !------------shanghui add for first_order_rho-----------------
  ...
  !real*8, dimension(:,:),  allocatable :: first_order_density_matrix_con_check
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
  ...
  ! logical :: inspector_runned = .false.
  ! common /prune_spm_dielectric_inspector/ inspector_runned
  ! begin work
 ...
 !------------shanghui end add for first_order_rho------------- 
  ...
  call prune_matrix_dielectric_maybe_init(n_my_batches_work)
  !-----------------------------------------------------------------------------
  ...
  do i_my_batch = 1, n_my_batches_work, 1
     ...
     if (prune_basis_once) then
        ...
        ! if (.not. inspector_runned) then
        !    allocate(batches_work(i_my_batch)%source_place(n_compute_c, n_compute_c))
        !    !print *, loc(batches_work(i_my_batch)%source_place(n_compute_c, n_compute_c))
        !    call prune_density_matrix_sparse_dielectric_inspector(n_compute_c, i_basis, batches_work(i_my_batch)%source_place)
        ! end if
        call prune_matrix_dielectric_dense_from_sparse(first_order_density_matrix_sparse, &
             first_order_density_matrix_con, n_compute_c, i_basis, i_my_batch)
             
        ! call prune_density_matrix_sparse_dielectric_executor(first_order_density_matrix_sparse, & 
        !      first_order_density_matrix_con, &
        !      n_compute_c, i_basis, batches_work(i_my_batch)%source_place)
     else
     ! end if
     ! if (.true.) then
     !-----------shanghui begin test prune_matrix here-------------------------
        call  prune_density_matrix_sparse_dielectric(first_order_density_matrix_sparse, & 
             first_order_density_matrix_con, &
             n_compute_c, i_basis)
        !print*, maxval(abs(first_order_density_matrix_con - first_order_density_matrix_con_check))
     end if
...
!---------------------------add for first_order_rho------------------------------------------             
             call  evaluate_first_order_rho_dielectric( &
...
!---------------------------end for first_order_rho------------------------------------------             
       ...
       i_full_points_DM_rho + batches_work(i_my_batch)%size) = (0.0d0,0.0d0)
...
end subroutine integrate_first_order_rho_dielectric
!******
