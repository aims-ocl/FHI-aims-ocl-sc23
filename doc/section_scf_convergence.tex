...
\keyword{elsi\_restart} -- please see that keyword for a
description. The 
\keyword{elsi\_restart} keyword is the supported restart functionality
going forward unless you have special reasons to prefer the
simple \keyword{restart} keyword. 

The \keyword{restart} keyword summarizes a set of different cases for
...
point. \keyword{restart}  comes in many internal variants and, for
example, restarting a 
calculation with more MPI tasks than a previous run is not
possible. The alternative \keyword{elsi\_restart} keyword (which uses
a different storage format than the \keyword{restart} keyword) is much
...
Note: A more generic restart functionality is provided by the
\keyword{elsi\_restart}, described further below. We recommend to use
\keyword{elsi\_restart} unless you have special reasons to prefer the
simple \keyword{restart} keyword. 
...
Only relevant when using keyword \keyword{restart}.
